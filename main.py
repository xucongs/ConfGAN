...
if args.train:
    ...
    with open(args.val_dataset, 'rb') as r:
        val_dataset = pickle.load(r)
    validation_batch = []
    for data_dict in val_dataset:
        batch_merge(data_dict)
        validation_batch.append(data_dict)

    for epoch in range(args.epoch):
        trainer.train(dataset_batch)  # Train for one epoch
        trainer.validate(validation_batch, epoch)  # Validate after each epoch
