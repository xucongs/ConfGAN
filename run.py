...
class GANTrainer:
    ...
    def train_step(self, data_dict):
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            # Generate fake distances using the generator
            ...
            # Calculate generator loss (related to L_G^WGA
            gen_loss = self.generator_loss(fake_output)
            # Calculate discriminator loss (related to L_D^WGAN)
    ...
    def validate(self, validation_dataset, epoch):
        ...
        for data_dict in validation_dataset:
            gen_dis = self.generator(data_dict, training=False)
            real_u = data_dict['distance']
            
            real_output = self.discriminator(data_dict, training=False)
            data_dict['distance'] = gen_dis
            fake_output = self.discriminator(data_dict, training=False)
        ...
        print(f"Epoch:{epoch}, Average Generator Loss: {avg_gen_loss}, Average Discriminator Loss: {avg_disc_loss}")
